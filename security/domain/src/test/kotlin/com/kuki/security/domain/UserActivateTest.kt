...
class UserActivateTest {
    ...
    @Test
    fun `user send activation request successful`() = runTest {
        val user = repository.load(userId.toString()) as User

        user.sendActivationRequest(email, Mock.activationTokenSender, Mock.otp())
        repository.save(user)
    }

    @Test
    fun `user try to send activation request with invalid email`() = runTest {
        val user = repository.load(userId.toString()) as User

        assertFailsWith<IllegalArgumentException> {
            user.sendActivationRequest(
                email = Email.fromString("john.doe2@email.com"),
                activationTokenSender = Mock.activationTokenSender,
                otp = Mock.otp()
            )
        }
    }
    ...
    @Test
    fun `user activate successful`() = runTest {
        val user = repository.load(userId.toString()) as User

        user.activate("token", Mock.otp())
        repository.save(user)
    ...
    @Test
    fun `user try to activate with invalid otp`() = runTest {
        val user = repository.load(userId.toString()) as User

        assertFailsWith<IllegalArgumentException> {
            user.activate("token", Mock.otp { _, _ -> false })
        }
    }
}
